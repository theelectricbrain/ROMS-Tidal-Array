#!/bin/bash
#
# svn $Id: build.bash 617 2012-05-17 17:23:11Z arango $
#::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
export   ROMS_APPLICATION=VALIDATION

# Set a local environmental variable to define the path to the directories
# where all this project's files are kept.

export        MY_ROOT_DIR=${HOME}/Desktop
...
#export       MY_ROMS_SRC=${MY_ROOT_DIR}/branches/arango
 export       MY_ROMS_SRC=${MY_ROOT_DIR}/ROMS_latest_patched
...
if [ -n "${USE_MY_LIBS:+1}" ]; then
  case "$FORT" in
    ...
    gfortran )
      ...
      export       ARPACK_LIBDIR=/usr/lib
      if [ -n "${USE_MPI:+1}" ]; then
        if [ "${which_MPI}" = "mpich" ]; then
          export        ESMF_DIR=/home/thomasr/Desktop/esmf
          export      MCT_INCDIR=/usr/include
          export      MCT_LIBDIR=/usr/lib
          export  PARPACK_LIBDIR=/usr/lib
        elif [ "${which_MPI}" = "mpich2" ]; then
          export        ESMF_DIR=/home/thomasr/Desktop/esmf
          export      MCT_INCDIR=/usr/include
          export      MCT_LIBDIR=/usr/lib
          ...
          export      MCT_INCDIR=/opt/MCT/include
          export      MCT_LIBDIR=/opt/MCT/lib
          export  PARPACK_LIBDIR=/usr/lib
        fi
      fi
      ...
      if [ -n "${USE_NETCDF4:+1}" ]; then
        if [ -n "${USE_PARALLEL_IO:+1}" ] && [ -n "${USE_MPI:+1}" ]; then
          if [ "${which_MPI}" = "mpich" ]; then
            export     NC_CONFIG=/usr/local/parallel/bin/nc-config
            export NETCDF_INCDIR=/usr/local/parallel/include
          elif [ "${which_MPI}" = "mpich2" ]; then
            export     NC_CONFIG=/usr/bin/nc-config
            export NETCDF_INCDIR=/opt/netcdf4-parallel/include
          elif [ "${which_MPI}" = "openmpi" ]; then
            export     NC_CONFIG=/usr/bin/nc-config
            export NETCDF_INCDIR=/opt/netcdf4-parallel/include
          fi
          ...
          export       NC_CONFIG=/usr/bin/nc-config
          export   NETCDF_INCDIR=/usr/include #/opt/netcdf4/include
          export   NETCDF_INCDIR=/usr/include #/opt/netcdf4/include
        fi
      else
        export     NETCDF_INCDIR=/usr/include #/opt/netcdf3/include
        export     NETCDF_LIBDIR=/usr/lib #/opt/netcdf3/lib
      fi
      ;;
