      MODULE mod_grid
!
!svn $Id: mod_grid.F 709 2014-01-23 20:09:38Z arango $
!================================================== Hernan G. Arango ===
...
!  imask_t    Mask at U or V points (0=no turbine, 1.0=turbine).       !
!  rmask_t    Mask at RHO-points (0=no turbine, Ct=turbine).           !
!  rmask1t    Mask at RHO-points (0=no turbine, Ctke=turbine).         !
!  rmask2t    Mask at RHO-points (0=no turbine, Cgls=turbine).         !
!  rmask3t    Mask at RHO-points (0=no turbine, Lcpa=turbine).         !
!  umask_t    Mask at U-points (0=no turbine, Ct=turbine).             !
!  vmask_t    Mask at V-points (0=no turbine, Ct=turbine).             !
...
! The following portion is Copyright (c) 2014 Thomas Roc and ITPower   !
          real(r8), pointer :: imask_t(:,:,:)
          real(r8), pointer :: rmask_t(:,:,:)
          real(r8), pointer :: rmask1t(:,:,:)
          real(r8), pointer :: rmask2t(:,:,:)
          real(r8), pointer :: rmask3t(:,:,:)
          real(r8), pointer :: umask_t(:,:,:)
          real(r8), pointer :: vmask_t(:,:,:)
!========================================== End of the portion =========
        END TYPE T_GRID
...
!================================================== Thomas Roc =========
! The following portion is Copyright (c) 2014 Thomas Roc and ITPower   !
      allocate ( GRID(ng) % imask_t(LBi:UBi,LBj:UBj,N(ng)) )
      allocate ( GRID(ng) % rmask_t(LBi:UBi,LBj:UBj,N(ng)) )
      allocate ( GRID(ng) % rmask1t(LBi:UBi,LBj:UBj,N(ng)) )
      allocate ( GRID(ng) % rmask2t(LBi:UBi,LBj:UBj,N(ng)) )
      allocate ( GRID(ng) % rmask3t(LBi:UBi,LBj:UBj,N(ng)) )
      allocate ( GRID(ng) % umask_t(LBi:UBi,LBj:UBj,N(ng)) )
      allocate ( GRID(ng) % vmask_t(LBi:UBi,LBj:UBj,N(ng)) )
!========================================== End of the portion =========
      RETURN
...
!================================================== Thomas Roc =========
! The following portion is Copyright (c) 2014 Thomas Roc and ITPower   !
             GRID(ng) % imask_t(i,j,k) = IniVal
             GRID(ng) % rmask_t(i,j,k) = IniVal
             GRID(ng) % rmask1t(i,j,k) = IniVal
             GRID(ng) % rmask2t(i,j,k) = IniVal
             GRID(ng) % rmask3t(i,j,k) = IniVal
             GRID(ng) % umask_t(i,j,k) = IniVal
             GRID(ng) % vmask_t(i,j,k) = IniVal
!========================================== End of the portion =========
            END DO
          END DO
          DO k=0,N(ng)
