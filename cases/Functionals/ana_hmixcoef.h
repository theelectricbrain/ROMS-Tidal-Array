...
!
!! svn $Id: ana_hmixcoef.h 585 2012-01-03 18:44:28Z arango $
!!================================================= Hernan G. Arango ===
...
!  Sponge areas.
!
      Jwrk = MAX(INT(user(1)), Mm(ng)/100)  ! same for sponge and nudging layers
      Iwrk = MAX(INT(user(1)),Lm(ng)/100)
!      Jwrk = INT(user(1))  ! same for sponge and nudging layers
!      Iwrk = INT(user(1))
...
!
      cff1=visc2(ng)
      cff2=20.0_r8 !multiplication coefficient for sponge viscosity
!
!  Southern edge.
!
!      DO j=JstrR,MIN(Jwrk,JendR)
!        DO i=IstrR,IendR
!        cff=1.0_r8+REAL(Jwrk-j,r8)*(cff2-1.0_r8)/REAL(Jwrk,r8)
!          visc2_r(i,j)=MAX(MAX(MIN(cff,cff2),1.0_r8)*visc2_r(i,j),visc2_r(i,j))
!          visc2_p(i,j)=MAX(MAX(MIN(cff,cff2),1.0_r8)*visc2_p(i,j),visc2_p(i,j))
!	  WRITE(*,*) 'visc2_r', visc2_r(i,j), 'visc2_p', visc2_p(i,j), 'i,j', i, j
!        END DO
!      END DO
!
!  Northern edge.
!
!      DO j=MAX(JstrR,Mm(ng)+1-Jwrk),JendR
!        cff=cff2-REAL(Mm(ng)+1-j,r8)*(cff2-1.0_r8)/REAL(Jwrk,r8)
!        DO i=IstrR,IendR
!          visc2_r(i,j)=MAX(MAX(MIN(cff,cff2),1.0_r8)*visc2_r(i,j),visc2_r(i,j))
!          visc2_p(i,j)=MAX(MAX(MIN(cff,cff2),1.0_r8)*visc2_p(i,j),visc2_p(i,j))
!	  WRITE(*,*) 'visc2_r', visc2_r(i,j), 'visc2_p', visc2_p(i,j), 'i,j', i, j
...
!        DO j=MAX(JstrR,i),MIN(Mm(ng)+1-i,JendR)
        DO j=JstrR,JendR
          cff=1.0_r8+REAL(Iwrk-i,r8)*(cff2-1.0_r8)/REAL(Iwrk,r8)
          visc2_r(i,j)=MAX(MAX(MIN(cff,cff2),1.0_r8)*visc2_r(i,j),visc2_r(i,j))
          visc2_p(i,j)=MAX(MAX(MIN(cff,cff2),1.0_r8)*visc2_p(i,j),visc2_p(i,j))
!	  WRITE(*,*) 'visc2_r', visc2_r(i,j), 'visc2_p', visc2_p(i,j), 'i,j', i, j
        END DO
...
!        DO j=MAX(JstrR,i),MIN(Mm(ng)+1-i,JendR)
        DO j=JstrR,JendR
        cff=cff2-REAL(Lm(ng)+1-i,r8)*(cff2-1.0_r8)/REAL(Iwrk,r8)
          visc2_r(i,j)=MAX(MAX(MIN(cff,cff2),1.0_r8)*visc2_r(i,j),visc2_r(i,j))
          visc2_p(i,j)=MAX(MAX(MIN(cff,cff2),1.0_r8)*visc2_p(i,j),visc2_p(i,j))
!	  WRITE(*,*) 'visc2_r', visc2_r(i,j), 'visc2_p', visc2_p(i,j), 'i,j', i, j
        END DO
